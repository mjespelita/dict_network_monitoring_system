...
@section('content')
    <div class='row'>
        <div class='col-lg-6 col-md-6 col-sm-12'>
            <h1>Trash Disconnecteddevices</h1>
        </div>
                ...
                <table class='table table-striped'>
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>Name</th><th>Device_name</th><th>Device_mac</th><th>Device_type</th><th>Status</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        @forelse($disconnecteddevices as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->name }}</td><td>{{ $item->device_name }}</td><td>{{ $item->device_mac }}</td><td>{{ $item->device_type }}</td><td>{{ $item->status }}</td>
                                <td>
                                    <a href='{{ route('disconnecteddevices.restore', $item->id) }}'><i class='fas fa-recycle text-info'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-restore').click(function () {
                ...
                $.post('/disconnecteddevices-restore-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    console.log(res)
