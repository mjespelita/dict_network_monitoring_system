...
@section('content')
    <div class='row'>
        <div class='col-lg-6 col-md-6 col-sm-12'>
            <h1>All Clientdetails</h1>
        </div>
        <div class='col-lg-6 col-md-6 col-sm-12' style='text-align: right;'>
            <a href='{{ url('trash-clientdetails') }}'><button class='btn btn-danger'><i class='fas fa-trash'></i> Trash <span class='text-warning'>{{ App\Models\Clientdetails::where('isTrash', '1')->count() }}</span></button></a>
            <a href='{{ route('clientdetails.create') }}'><button class='btn btn-success'><i class='fas fa-plus'></i> Add Clientdetails</button></a>
        </div>
    </div>
                    ...
                    <tbody>
                        @forelse($clientdetails as $item)
                            <tr>
                                ...
                                <td>
                                    <a href='{{ route('clientdetails.show', $item->id) }}'><i class='fas fa-eye text-success'></i></a>
                                    <a href='{{ route('clientdetails.edit', $item->id) }}'><i class='fas fa-edit text-info'></i></a>
                                    <a href='{{ route('clientdetails.delete', $item->id) }}'><i class='fas fa-trash text-danger'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-move-to-trash').click(function () {
                ...
                $.post('/clientdetails-move-to-trash-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    window.location.reload();
